...
def get_petitioners_and_respondents_speakers(text):
    ...
    ########## I think there is room to improve here. As is, if 'pet' is in the set and 'res' is not, all 'others' are converted to 'res'.
    ########## Also, there is power in the order in which they appear. They always appear Pet first, then any amici supporting, then Res, then amici supporting, then possibly rebuttal by Pet.
    ########## This ordering could be leveraged.
    ########## When do neutral amici appear? Between them? Whose time do they take?
