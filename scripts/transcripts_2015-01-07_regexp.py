...
#         end = text.find('P R O C E E D')
    #end = (re.search(r'[Pp]\s*[Rr]]\s*[Oo]\s*[Cc]\s*[Ee]\s*[Ee]\s*[Dd]', text)).start()
    ...
    #print(names_text)
    #print(names_text)
    names_list = re.findall(r'^([A-Z\.\,\s]+)\,\s[Ee][Ss][Qq]', names_text, flags=re.M)
    if len(names_list) < 2: print(names_list)
    #for n in names_list: 
    #    ## Remove 
    #    print(n,'\n')
    ...
    end2 = text.find('PROCEED')
    if end2 == -1:
        end2 = text.find('P R O C E E D')
    if end2 == -1:
        end2 = text.find('JUSTICE ROBERTS')
    names_text_2 = text[end:end2]
    #print(names_text_2)
    split_names_2 = names_text_2.split('ARGUMENT OF')
    ## For each of these groups, extract the name and the side
    for s in split_names_2: 
        #print(s)
        if ', ESQ' in s:
            n = s.split(', ESQ')[0]
            n = n.split(',')[0]
    ...
    names_list_2 = re.findall(r'(.*)\,\s[Ee][Ss][Qq]', names_text_2, flags=re.M)  
    #for n in names_list_2: print(n)  
    ## Extract the last name, watching out for ', Jr.' and ', III' and maybe others?
    for n in names_list:
        #print(n)
        n = n.split(',')[0]
        n = n.split()[-1]
...
def main():
    ...
    for year in years:
        ...
        for file in files:
            ...
            ## Change all pesky 'McGREGOR' 'McCONNELL and 'DiNARDO' to all caps. O'Connnor?
            ...
            for w in words: print(w, sum(words[w]))

            for x in sides: 
                #if x in words: 
                print(x, sides[x], sum(words[x])) 
            ...
            for x in sides:
                if x in words:
                    d[sides[x]] += sum(words[x])
            for x in d: print(x, d[x])
            ######## TO DO:
            ######## fix O'Connell apostrophe errors
            ######## see if the side that talks more wins more.
    ...
    print('(winning_side, more_lawyers, most_interrupted)') 
    #for fac in factors:
